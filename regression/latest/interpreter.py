...
where
    ...
    n  [bytes | FLOP]  work metric — bytes for comms, FLOPs for MatMul-family
"""
...
class Interpreter:
    ...
    def _iqr_filter(x_vals, y_vals):
        ...
        We only filter the upper tail because the lower tail
    ...
    def _fit_hockney(self, lane, key, x_vals, y_vals, any_fused=False):
        ...
        if slope <= 1e-12:
            ...
            return alpha, float("inf"), r2, True, "blah blah blah"
