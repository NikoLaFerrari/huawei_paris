...
class Predictor:
    ...
    def predict_with_breakdown(self, pred_dims):
        ...
        """
        print("[predictor] predict_with_breakdown(...)")
        known_lanes = {
            "COMPUTE", "BUBBLE",
    ...
    def predict_from_lane_totals(self, base_cd, pred_dims):
        ...
        """
        print("[predictor] predict_from_lane_totals(...)")
        lane_times = {}
    ...
    def _compute_scales(self, lane, pred_dims, f, pass_type, variable):
        ...
        others   :  1, 1, 1
        """
        print(f"[debug] _compute_scales({lane},{pred_dims},{f},{pass_type},{variable}")
        b = self.base_dims
        p = pred_dims
        ...
        mb_t = _get(p, "mbs", "MBS") if any(k in p for k in ("mbs", "MBS")) else mb_b
        ...
        if lane == "DP_COMM":
            ...
            alpha_scale = 1.0
            beta_scale  = 1.0
            n_scale     = (pp_t / pp_b) #(dp_b / dp_t) * (mp_b / mp_t)

        elif lane == "COMPUTE":
            alpha_scale = 1.0
            beta_scale  = 1.0
            if pass_type == "recompute":
                n_scale = (mb_t / mb_b) * (pp_t / pp_b) * (mp_b / mp_t) * (dp_b / dp_t) 
            else:
                n_scale = (mb_t / mb_b) * (dp_b / dp_t) * (mp_b / mp_t) 
            n_scale = 1.0
            ...
            beta_scale  = 1.0
            n_scale     = 1.0
        print(f"[debug] return: {alpha_scale}, {n_scale}, {beta_scale}")
