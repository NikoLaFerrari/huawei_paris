...
class Handler:
    ...
    def calibrate_coefficients(self, actual_dur):
        ...
        for dims in self.meta['trace_dims_list']:
            _, breakdown = temp_pred.predict_with_breakdown(dims)
    ...
    def get_strats_from_nd(self):
        ...
        gbs = raw.get('runner_config').get('batch_size')
    ...
    def run_calibration(self):
        ...
        for i, strat in enumerate(nd_strats):#self.meta['trace_dims_list'])):
            vals = [int(v) for v in strat.values()]
            pred_dims = {keys[j]: vals[j] for j in range(len(keys))}
            total_work, pred_buckets = predictor.predict_with_breakdown(pred_dims)

            r_out = {
                lane: (val / total_work) * 100 if total_work > 0 else 0
                for lane, val in pred_buckets['bucket_times'].items()
            }
