...
class Extractor:
    ...
    def run_extractor(self):
        ...
        for i in range(len(self.trace_paths)):
            ...
            # Must happen before iterating events so that warmup data is
            # excluded from both structured_data and classification totals.
            ...
            # Pass windows so that classification totals also reflect only
            # steady-state steps and remain consistent with structured_data.
    ...
    def _get_steady_state_windows(process_info, warmup_steps=2):
        ...
        Why warmup must be excluded
        ───────────────────────────
        The first 1–2 steps of a MindSpore trace contain:
          - ACL/CUDA graph capture overhead
          - Cold HBM cache misses
          - JIT kernel compilation

        Their primitive durations are systematically inflated compared to
        steady state. Including them in regression raises every α (intercept)
        and biases β, causing predictions to overestimate step time.

        Parameters
        ----------
        process_info : dict   parsed trace (from prof.parse_process_info)
        warmup_steps : int    number of leading steps to discard (default 2)

        Returns
        -------
        list of (float, float) or None
            None means no step boundary info was found — keep all events.
        """
    ...
    def _event_in_windows(event, windows):
        ...
        We check the event start timestamp (ts) rather than the full
        [ts, ts+dur] interval to avoid rejecting events that straddle a
        step boundary due to small timestamp skew.
        """
        if windows is None:
            return True
        ts = event.get("ts", 0.0)
    ...
    def _get_classification(self, process_info, graph_scope_map, parallel_dims,
        ...
        Parameters
        ----------
        process_info    : dict   parsed trace
        graph_scope_map : dict   scope → Operator lookup
        parallel_dims   : dict   parallel config from YAML
        windows         : list of (float, float) | None
            Steady-state step intervals from _get_steady_state_windows().
            Events outside these intervals are skipped so that classification
            totals are consistent with the structured_data built in
            run_extractor(). Pass None to keep all events.

        Returns
        -------
        (raw_cd_dict, pctg_dict)
        """
        cd = {
            "COMPUTE":      0.0,
            "UNKNOWN_COMM": 0.0,
